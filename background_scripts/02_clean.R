...
pa <- read_rds("data/CPCAD_Dec2024_BC_fixed.rds") %>%
...
pa_wha <- wha %>%
  ...
  st_point_on_surface() %>%
  st_join(
    dplyr::filter(pa, name_e == "Wildlife Habitat Areas") %>%
...
pa_ogma <- ogma %>%
  ...
  dplyr::filter(!is.na(legalization_frpa_date)) %>%
  st_cast(to = "POLYGON", warn = FALSE) %>%
  st_point_on_surface() %>%
  st_join(
    dplyr::filter(pa, name_e == "Old Growth Management Areas (Mapped Legal)") %>%
      tibble::rownames_to_column(), .
  ) %>%
  group_by(rowname) %>%
...
pa <- pa %>%
  mutate(
    ...
    park_type =  case_when(pa_oecm_df == 1 ~ "PA",
                           pa_oecm_df == 2 ~ "OECM",
                           pa_oecm_df == 3 ~ "PA",
                           pa_oecm_df == 4 ~ "OECM")) %>%
  arrange(desc(pa_oecm_df), iucn_cat, date, area_all)

# Save file for comparisons
write_rds(pa, "data/CPCAD_Dec2024_BC_clean.rds")
...
write_rds(pa_mult, "data/CPCAD_Dec2024_BC_clean_no_ovlps.rds")
